...
<style>
    ...
    .kvvw-layout {
        ...
        gap: 2rem;
        /*
         * FIX: padding-bottom para que la barra mobile (56px aprox)
         * no tape los últimos productos ni el botón load more.
         */
        padding-bottom: 6rem;
    }
                ...
                <div class="kvvw-layout">
                    ...
                    <div class="kvvw-main">
                            ...
                            <template v-else>
                                ...
                                <template v-else>
                                    <div class="kvvw-empty">
                                        <img style="width:5rem;height:5rem;opacity:0.35;" src="{{ bagisto_asset('images/thank-you.png') }}" alt="@lang('shop::app.categories.view.empty')" />
                        ...
                        <div class="kvvw-load-more-wrap">
                            ...
                            <button class="kvvw-load-more--loading" v-else-if="links.next">
        ...
        <script type="module">
            app.component('v-category', {
                ...
                watch: {
                    ...
                    // ── NUEVO ──────────────────────────────────────────────────────────
    // Emite eventos DOM personalizados cuando un drawer mobile se abre
    // o cierra. Solo actúa en mobile (isMobile). El prefijo 'kv-drawer'
    // garantiza que no colisiona con ningún evento de Bagisto.
    isDrawerActive: {
        deep: true,
        handler(val) {
            if (! this.isMobile) return;

            const anyOpen = val.filter || val.toolbar;

            window.dispatchEvent(
                new CustomEvent(anyOpen ? 'kv-drawer-open' : 'kv-drawer-close')
            );
        }
    },
    // ── FIN NUEVO ──────────────────────────────────────────────────────
                ...
                methods: {
                    ...
                    /*
                     * FIX: método dedicado para abrir drawers.
                     * Activa el drawer deseado y deja que el componente
                     * v-drawer maneje el body overflow correctamente.
                     * Antes se mutaba isDrawerActive directamente en el
                     * template con @click="isDrawerActive.filter = true"
                     * lo que en algunos casos no disparaba el open() del
                     * v-drawer y el body overflow quedaba sin bloquear.
                     */
                    openDrawer(type) {
                        this.isDrawerActive = {
                            toolbar: false,
                        ...
                        this.$nextTick(() => {
                            this.isDrawerActive[type] = true;
                        });
                    },

                    getProducts() {
                        /*
                         * FIX CRÍTICO: se reemplazó document.body.style.overflow = 'scroll'
                         * por '' (vacío) para dejar que el navegador use el valor por
                         * defecto, que es auto/visible según el CSS global.
                         *
                         * 'scroll' forzaba una barra de scroll permanente y además
                         * interfería con la detección de drawers abiertos en layout.blade.php
                         * (que usa overflow === 'hidden' como señal).
                         *
                         * Cerramos los drawers vía isDrawerActive antes de cargar
                         * para que v-drawer emita el evento close() y limpie el
                         * body overflow él mismo.
                         */
                        this.isDrawerActive = {
                            toolbar: false,
                            filter: false,
