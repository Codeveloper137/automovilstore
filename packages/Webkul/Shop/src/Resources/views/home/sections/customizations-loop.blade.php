...
<section class="kv-section" style="background:linear-gradient(180deg,var(--background) 0%,rgba(99,102,241,0.025) 50%,var(--background) 100%);">
  ...
  <div class="kv-section-inner">
    ...
    @php
    $childCategories = app(\Webkul\Category\Repositories\CategoryRepository::class)
    ->getVisibleCategoryTree(core()->getCurrentChannel()->root_category_id);
    @endphp

    @if ($childCategories->isNotEmpty())
    <x-shop::categories.carousel :title="''" :categories="$childCategories" :navigation-link="route('shop.home.index')" />
    @else
    <p class="text-center text-gray-400 py-8">No hay categorías disponibles.</p>
    @endif
  </div>
</section>
<hr class="kv-divider">
