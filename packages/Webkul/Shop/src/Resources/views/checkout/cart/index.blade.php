...
<style>
    ...
    /* ── Header del cart ──────────────────────────────────── */
    /*
     * FIX CRÍTICO: backdrop-filter directamente en un elemento
     * sticky/fixed crea un stacking context que atrapa todos los
     * hijos con position:fixed (modales de login, etc.).
     *
     * Solución idéntica al header principal:
     * el blur vive en ::before, no en el elemento.
     */
    .kv-cart-header {
        position: sticky;
        top: 0;
        z-index: 40;
        /* SIN backdrop-filter aquí */
        background: transparent;
        border-bottom: 1px solid rgba(226,232,240,0.6);
        box-shadow: 0 2px 20px rgba(99,102,241,0.06);
    }
    .kv-cart-header::before {
        content: '';
        position: absolute;
        inset: 0;
        z-index: -1;
    ...
    /* Modal de login — siempre por encima del header */
    [x-ref="modal"],
    [x-ref="backdrop"] {
    ...
    .kv-cart-btn-primary:focus-visible {
    ...
    .kv-cart-btn-secondary:focus-visible {
        outline: 2px solid #6366f1;
        outline-offset: 3px;
    }
    ...
    .kv-empty-cart {
        ...
        animation: kvCartFadeUp 0.6s ease-out both;
    }
    ...
    .kv-cart-error {
        ...
        font-size: 0.875rem; font-weight: 500; color: #c2410c;
        margin-top: 1.25rem;
    ...
    <div class="kv-cart-root">
        ...
        <div class="flex-auto">
            <div class="relative z-10 mx-auto max-w-[1400px] px-16 max-lg:px-8 max-md:px-4">
                ...
                @php $errors = Cart::getErrors(); @endphp
    ...
    <script type="text/x-template" id="v-cart-template">
        <div>
            ...
            <template v-else>
                    ...
                    <div class="flex flex-1 flex-col gap-5 max-md:gap-4">
                        ...
                        <div class="flex items-center justify-between rounded-xl border border-[rgba(226,232,240,0.6)] bg-[rgba(255,255,255,0.55)] px-4 py-3 backdrop-blur-sm max-md:px-3 max-md:py-2">
                            <div class="flex select-none items-center gap-2.5">
                                ...
                                <label class="icon-uncheck peer-checked:icon-check-box cursor-pointer text-2xl text-[rgba(99,102,241,0.45)] peer-checked:text-[#6366f1] transition-colors"
                                       for="select-all" tabindex="0"
                                       aria-label="@lang('shop::app.checkout.cart.index.select-all')"></label>
                            ...
                            <div class="flex items-center gap-3" v-if="selectedItemsCount">
                                <button class="flex items-center gap-1.5 text-xs font-semibold text-[var(--muted-foreground)] transition-colors hover:text-[#ef4444]"
                                        role="button" tabindex="0" @click="removeSelectedItems">
                                ...
                                @if (auth()->guard()->check())
                                    ...
                                    <button class="flex items-center gap-1.5 text-xs font-semibold text-[var(--muted-foreground)] transition-colors hover:text-[#6366f1]"
                                            role="button" tabindex="0" @click="moveToWishlistSelectedItems">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        <!-- Lista de items -->
                        <div class="grid gap-y-4" v-for="item in cart?.items">
                            <div class="kv-cart-item">
                                <div class="flex items-start justify-between gap-3">

                                    <!-- Izquierda: checkbox + imagen + info -->
                                    <div class="flex items-start gap-4 max-md:gap-3">

                                        <!-- Checkbox -->
                                        <div class="mt-12 select-none max-md:mt-9 max-sm:mt-7">
                                            <input type="checkbox" :id="'item_' + item.id" class="peer hidden"
                                                   v-model="item.selected" @change="updateAllSelected">
                                            <label class="icon-uncheck peer-checked:icon-check-box cursor-pointer text-2xl text-[rgba(99,102,241,0.40)] peer-checked:text-[#6366f1] transition-colors"
                                                   :for="'item_' + item.id" tabindex="0"
                                                   aria-label="@lang('shop::app.checkout.cart.index.select-cart-item')"></label>
                                        </div>
                                        ...
                                        <div class="grid place-content-start gap-y-2 max-md:gap-y-1">
                                            ...
                                            <div class="grid select-none gap-x-2 gap-y-1" v-if="item.options.length">
                                                <p class="flex cursor-pointer items-center gap-2 text-xs font-medium text-[var(--muted-foreground)] transition-colors hover:text-[#6366f1]"
                                                ...
                                                <div class="grid gap-1.5 rounded-lg bg-[rgba(99,102,241,0.04)] px-3 py-2"
                                                     v-show="item.option_show">
                                                    <template v-for="option in item.options">
                                                        <div class="flex items-center gap-1.5 max-md:grid max-md:gap-0">
                                                            <p class="text-xs font-semibold text-[var(--muted-foreground)]">@{{ option.attribute_name + ':' }}</p>
                                                            <p class="text-xs text-[var(--foreground)]">@{{ option.option_label }}</p>
                                                        </div>
                                                    </template>
                                                </div>
                                            </div>
                                    ...
                                    <!-- Derecha: precio + eliminar (desktop) -->
                                    <div class="flex flex-col items-end gap-2 text-right max-md:hidden">
                                        ...
                                        <template v-if="displayTax.prices == 'including_tax'">
                                            <p class="text-lg font-bold text-[#6366f1]">@{{ item.formatted_total_incl_tax }}</p>
                                        </template>
                                        ...
                                        <template v-else>
                                            <p class="text-lg font-bold text-[#6366f1]">@{{ item.formatted_total }}</p>
                                        </template>
                        ...
                        <div class="flex flex-wrap items-center justify-end gap-4 pt-2 max-md:justify-between max-md:gap-3">
                            ...
                            <a class="kv-cart-btn-secondary max-md:text-xs"
                ...
                <!-- ── CARRITO VACÍO ────────────────────────── -->
                <div class="kv-empty-cart" v-else>
                    ...
                    <img class="hidden" src="{{ bagisto_asset('images/thank-you.png') }}"
                    ...
                    <a href="{{ route('shop.home.index') }}" class="kv-cart-btn-primary mt-8">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        app.component('v-cart', {
            ...
            methods: {
                    ...
                    this.$emitter.emit('open-confirm-modal', {
                        agree: () => {
                            this.$axios.post('{{ route('shop.api.checkout.cart.destroy') }}', {
                                '_method': 'DELETE', 'cart_item_id': itemId,
                            })
                            .then(response => {
                                this.cart = response.data.data;
                                this.$emitter.emit('add-flash', { type: 'success', message: response.data.message });
                ...
                removeSelectedItems() {
                    this.$emitter.emit('open-confirm-modal', {
                        agree: () => {
                            const ids = this.cart.items.flatMap(item => item.selected ? item.id : []);
                ...
                moveToWishlistSelectedItems() {
                    this.$emitter.emit('open-confirm-modal', {
                        agree: () => {
                            const ids = this.cart.items.flatMap(item => item.selected ? item.id : []);
                            const qty = this.cart.items
                                .filter(item => item.selected)
                                .map(item => this.applied.quantity[item.id] ?? item.quantity);
                            this.$axios.post('{{ route('shop.api.checkout.cart.move_to_wishlist') }}', { 'ids': ids, 'qty': qty })
                                .then(response => {
                                    this.cart = response.data.data;
                                    this.$emitter.emit('update-mini-cart', response.data.data);
                                    this.$emitter.emit('add-flash', { type: 'success', message: response.data.message });
