...
<style>
    ...
    /* ── BUG FIX #3: Image zoomer — touch events y z-index ── */
    .image-zoomer,
    [class*="zoomer"] {
        touch-action: manipulation;
    }

    /* Asegurar z-index del zoomer por encima de todo (cart drawer = 999) */
    [class*="zoomer"].fixed,
    .image-zoomer.fixed {
        z-index: 10000 !important;
    }

    /* Botones dentro del zoomer reciben eventos táctiles */
    [class*="zoomer"] button,
    [class*="zoomer"] [role="button"],
    .image-zoomer button {
        touch-action: manipulation;
...
<script type="text/x-template" id="v-product-gallery-template">
    <div>
        ...
        <!-- BUG FIX #3: Añadido @close para capturar el evento de cierre del zoomer
             y sincronizar el estado isImageZooming en el componente padre -->
        <x-shop::image-zoomer
            ::attachments="attachments"
            ::is-image-zooming="isImageZooming"
            ::initial-index="`media_${activeIndex}`"
            @close="isImageZooming = false"
        />
    </div>
</script>
...
<script type="module">
    app.component('v-product-gallery', {
        ...
        watch: {
            ...
            // BUG FIX #3: Controlar overflow del body cuando el zoomer está abierto
            isImageZooming(val) {
        ...
        mounted() {
            ...
            // BUG FIX #3: Cerrar zoomer con tecla Escape en mobile
            document.addEventListener('keydown', (e) => {
                if (e.key === 'Escape' && this.isImageZooming) {
                    this.isImageZooming = false;
                }
            });
        },
        ...
        methods: {
            ...
            change(media, index) {
                ...
                this.isImageZooming = false; // BUG FIX #3: cerrar zoomer al cambiar imagen
